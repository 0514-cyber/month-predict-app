...
# --- データ読み込み（ファイルパスは固定） ---
def load_all_series():
    def load_single_series(xlsx_path, col_name):
        xls = pd.ExcelFile(xlsx_path)
        df_all = []
        for sheet in reversed(xls.sheet_names):
            try:
                df = pd.read_excel(xlsx_path, sheet_name=sheet, usecols=[0, 1])
...
def summarize_month(df_feat, feature_cols, month_start, month_end):
    ...
    if not results:
        return "⚠️ この月の予測に必要なデータが不足しています。"
...
# --- Streamlit UI ---
st.set_page_config(page_title="月予測アプリ", page_icon="📅")
st.title("📅 月単位の未来予測アプリ")
...
if st.button("月予測を実行"):
    summary = summarize_month(df_feat, feature_cols, month_start, month_end)
    st.markdown(summary)
