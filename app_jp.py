...
def summarize_month(df_feat, feature_cols, month_start, month_end):
    ...
    direction = "📈 Upward Trend" if avg_pred > 0 else "📉 Downward Trend"

    summary = f"""
    ## Monthly Forecast Summary
    - Average Prediction: `{avg_pred:.5f}`
    - Direction: {direction}
    """
    return pd.DataFrame({"Date": dates, "Prediction": results}).set_index("Date"), summary
...
selected_month = st.selectbox("Select the month you want to forecast", available_months)
...
st.caption(f"📌 Date of Latest Data: {latest_date.date()}")